...
try:
    from llm.food_plan_generator import FoodPlanGenerator as MealPlanGenerator
except Exception:
...
async def generate_meal_plan(payload: GenerateMealPlanRequest, auth: AuthContext = Depends(require_auth_context)):
...
async def generate_meal_plan_internal(payload: GenerateMealPlanRequest, auth: dict = Depends(require_internal_auth)):
