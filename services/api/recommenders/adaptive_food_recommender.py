...
class AdaptiveFoodRecommender:
    ...
    def recommend_food(
        ...
        food_characteristics = cls._determine_food_characteristics(
            nutrition_dna, food_type, context, enhanced_profile
        )

        # Find matching meal template
        food_template = cls._select_food_template(food_characteristics, food_type, constraints)

        # Calculate nutrition values
        nutrition = cls._calculate_food_nutrition(food_template, nutrition_dna, food_type)

        # Determine optimal timing
        optimal_time = cls._calculate_optimal_timing(nutrition_dna, food_type, context)

        # Generate reasoning
        reasoning = cls._generate_food_reasoning(nutrition_dna, food_template, context, food_characteristics)

        return PersonalizedFoodRecommendation(
            food_type=food_type,
            recommended_time=optimal_time,
            matches_energy_level=food_characteristics.get('energy_match', False),
            addresses_typical_craving=food_characteristics.get('craving_match', False),
            fits_schedule_pattern=food_characteristics.get('schedule_match', True),
            supports_current_goal=food_characteristics.get('goal_match', True),
            dish_name=food_template['name'],
            description=cls._generate_food_description(food_template, food_characteristics),
            reasoning=reasoning,
            ...
            prep_time_minutes=food_template['prep_time'],
            difficulty_level=cls._assess_difficulty(food_template, nutrition_dna),
            ingredients=cls._generate_ingredients_list(food_template, nutrition['calories'])
        )

    @classmethod
    def _determine_food_characteristics(
        cls,
        nutrition_dna: NutritionDNA,
        food_type: str,
        ...
        if preference_flags.get('stress_sensitive'):
            ...
            if characteristics['stress_level'] == 'high':
                characteristics['food_style'] = 'comfort_healthy'
                characteristics['avoid_complex'] = True
            else:
                characteristics['food_style'] = 'balanced'
        ...
        if nutrition_dna.archetype == EatingPersonality.STRESS_DRIVEN:
            ...
            if characteristics['time_constraint'] == 'limited':
                characteristics['food_style'] = 'quick_protein'
            else:
                characteristics['food_style'] = 'balanced'
            ...
            if not characteristics['social_context']:  # eating with others
                characteristics['food_style'] = 'social_friendly'
            else:
                characteristics['food_style'] = 'balanced'

        elif nutrition_dna.archetype == EatingPersonality.EARLY_BIRD_PLANNER:
            characteristics['food_style'] = 'energy_boosting' if food_type == 'breakfast' else 'balanced'
        ...
        if 'food_style' not in characteristics:
            characteristics['food_style'] = 'balanced'

        return characteristics

    @classmethod
    def _select_food_template(
        cls,
        ...
        food_style = characteristics.get('food_style', 'balanced')

        # Get templates for the meal style
        style_templates = cls.FOOD_TEMPLATES.get(food_style, {})
        food_options = style_templates.get(food_type, [])

        # If no specific style templates, use quick_protein as fallback
        if not food_options:
            food_options = cls.FOOD_TEMPLATES.get('quick_protein', {}).get(food_type, [])
    ...
    def _calculate_food_nutrition(
        ...
        food_calories = int(daily_calories * food_calorie_ratios.get(food_type, 0.25))

        # Adjust for meal characteristics
        protein_ratio = food_template.get('protein_ratio', 0.25)

        # Calculate macronutrients
        protein_calories = food_calories * protein_ratio
        protein_grams = int(protein_calories / 4)  # 4 calories per gram of protein

        fat_calories = food_calories * 0.30  # 30% from fats
        fat_grams = int(fat_calories / 9)  # 9 calories per gram of fat

        carb_calories = food_calories - protein_calories - fat_calories
        carb_grams = int(carb_calories / 4)  # 4 calories per gram of carbs

        # Fiber estimate (roughly 12-15g per 1000 calories)
        fiber_grams = int(food_calories / 1000 * 13)
    ...
    def _calculate_optimal_timing(
        ...
        if food_type in schedule:
            # If user has specific schedule constraints, respect them
            scheduled_time = schedule[food_type]
        ...
        if nutrition_dna.archetype == EatingPersonality.LATE_STARTER_IMPULSIVE:
            ...
            if food_type == 'breakfast' and base_time.hour < 8:
                base_time = time(8, 30)
    ...
    def _generate_food_reasoning(
        ...
        protein_ratio = food_template.get('protein_ratio', 0.2)
    ...
    @classmethod
    def _generate_food_description(cls, food_template: Dict[str, Any], characteristics: Dict[str, Any]) -> str:
        """Generate appealing description of the meal"""

        name = food_template['name']
        prep_time = food_template['prep_time']
        ...
        food_style = characteristics.get('food_style', 'balanced')
        base_description = descriptions.get(food_style, f"Сбалансированное {name}")
    ...
    @classmethod
    def _assess_difficulty(cls, food_template: Dict[str, Any], nutrition_dna: NutritionDNA) -> str:
    ...
    def _generate_ingredients_list(cls, food_template: Dict[str, Any], calories: int) -> List[Dict[str, Any]]:
        ...
        elif 'овсянка' in food_name or 'каша' in food_name:
            ingredients.extend([
                {'name': 'Овсяные хлопья', 'amount': 50, 'unit': 'г'},
                {'name': 'Молоко', 'amount': 200, 'unit': 'мл'},
                {'name': 'Ягоды', 'amount': 80, 'unit': 'г'}
            ])
        elif 'курица' in food_name or 'куриная' in food_name:
            ingredients.extend([
    ...
    def generate_daily_food_plan(
        ...
        constraints: Dict[str, Any] = None,
        enhanced_profile: Dict[str, Any] = None
    ) -> Dict[str, PersonalizedFoodRecommendation]:
        ...
        for food_type in food_types:
            ...
            food_context = day_context.copy()

            if food_type == 'breakfast':
                food_context['current_time'] = nutrition_dna.temporal_patterns.preferred_breakfast_time
            elif food_type == 'lunch':
                food_context['current_time'] = nutrition_dna.temporal_patterns.preferred_lunch_time
            else:  # dinner
                food_context['current_time'] = nutrition_dna.temporal_patterns.preferred_dinner_time

            foods[food_type] = cls.recommend_food(
                nutrition_dna, food_type, food_context, constraints, enhanced_profile
            )
    ...
    def generate_adaptive_suggestions(
        ...
        if recent_foods:
            # Check variety
            recent_dishes = [food.get('dish_name', '') for meal in recent_foods[-7:]]  # Last 7 meals
            ...
            prep_times = [food.get('prep_time_minutes', 30) for meal in recent_foods[-5:]]
