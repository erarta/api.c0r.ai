#!/usr/bin/env python3
"""
Simple Migration Runner for Enhanced Nutrition System
Runs migration directly without complex tracking
"""
...
def main():
    ...
    print("🧬 Enhanced Nutrition System - Simple Migration Runner")
    print("=" * 60)

    if len(sys.argv) < 2:
        print("Usage: python run_migration_simple.py <environment> [migration_file]")
        print("  environment: 'development' or 'production'")
        print("  migration_file: optional specific migration file name")
        print("")
        print("Examples:")
        print("  python run_migration_simple.py development")
        print("  python run_migration_simple.py production")
        print("  python run_migration_simple.py development 2025-09-25_rename_meal_to_food.sql")
        sys.exit(1)
    ...
    log(f"Running migration for {environment} environment")
    log(f"Using configuration file: {env_file}")
    print("")

    # Confirm before running
    if environment == 'production':
        response = input("⚠️  You are about to run migration on PRODUCTION database. Continue? (yes/no): ")
    ...
    if success:
        ...
        if verify_migration(env_file):
            print("")
            log("🎉 Enhanced Nutrition System is ready!")
            log("Next steps:")
