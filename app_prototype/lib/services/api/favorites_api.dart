...
class FavoritesApi {
  ...
  Future<List<Map<String, dynamic>>> list({int limit = 20, String? search}) async {
    final res = await _client.dio.get('/v1/app/favorites', queryParameters: {
      'limit': limit,
  ...
  Future<Map<String, dynamic>> save({required String name, required Map<String, dynamic> itemsJson}) async {
    final res = await _client.dio.post('/v1/app/favorites', data: {
      'name': name,
  ...
  Future<void> delete(String id) async {
    await _client.dio.delete('/v1/app/favorites/$id');
  }
}
